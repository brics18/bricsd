...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("bricsd-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("bricsd datadir: %s", dataDir)

	kaspadCmd, err := common.StartCmd("BRICSD",
		"bricsd",
	...
	spawn("startNode-kaspadCmd.Wait", func() {
		...
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("bricsdCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes kaspad closes ungracefully
				log.Errorf("bricsdCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(kaspadCmd, "bricsdCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	spawn("brics-miner-Cmd.Wait", func() {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(kaspaMinerCmd, "bricsMinerCmd")
	return nil
}
